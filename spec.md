...
| # | タスク | ブロック |
|---|---|---|
| 31 | [文字の部屋（未完成・動作不安定）](#31-文字の部屋-未完成動作不安定) | L |
| 33 | [粉の部屋（未完成・動作不安定）](#33-粉の部屋-未完成動作不安定) | L |

#### 保留中（外部判断待ち・再有効化待ち）
| # | タスク | ブロック | 理由 |
|---|---|---|---|
| 43 | [部屋入退室通知に部屋名を表示（ID→部屋名）](#43-部屋入退室通知に部屋名を表示--2026-06-04-実装完了未確認) | F / index.js | 実装済みだが動作未確認 |
| 3 | [読み上げ機能（TTS）続き](#3-読み上げ機能) | index.js | VoiceVox導入可否がサーバースペック次第。公開サーバー移行後に判断 |
| 27 | [ボイスチェンジャー（配信中に使用可能）](#27-ボイスチェンジャー) | index.js | ピッチ変換はライブラリ or AudioWorklet必須で重め。公開後に要件確認 |
| 7 | [時間帯フィルター（夕方・夜の色調）](#7-エントランス草原-時間帯フィルター保留中) | index.js | 実装済みだが無効化中。再有効化は `skyTime: true` を戻すだけ |
| 28 | [動画数変更時に位置をリセット（videoAutoReset）](#53-動画数変更時に位置をリセットvideoautoreset) | index.ejs / index.js | コメントアウト中。有効化時は index.ejs のチェックボックスのコメントアウトを外す。CLAUDE.md の localStorage キー一覧に `videoAutoReset` を追記すること |

#### 後回し
| # | タスク | ブロック |
|---|---|---|
| 26 | [YouTuberモード（人物切り抜き・右下表示）](#26-youtuberモード) | index.js |
| 21 | [動画コメント機能（ニコニコ風、動画上に流れるコメント）](#21-動画コメント機能) | index.js |
| 12 | [リスナー人数表示（配信画面に受信者数を表示）](#12-リスナー人数表示) | index.js / F |

#### 別枠（最後・横断的調査が必要）

| # | タスク | ブロック |
|---|---|---|
| 51 | [パフォーマンス改善（重い要素の洗い出し・改善）](#51-パフォーマンス改善別枠) | K |
| 52 | [セキュリティ改善（問題の洗い出し・修正）](#52-セキュリティ改善別枠) | K |
...
**2026-06-20 修正**
- iOS Safari の autoplay policy で TTS が鳴らない問題を修正
  - `pointerdown` リスナー（初回タップ時）で `ttsAudioCtx` を生成＋`resume()` → AudioContext がアンロックされ、以降の socket 経由 `speakText()` が iOS でも再生される
...
### 5. エントランスの追加及び、 1F/2F レイヤーシステム ✅ (2026-04-12 実装完了)
...
- アバターカラーが真っ黒（`r+g+b < 60`）の場合は黒背景・白文字
- それ以外は背景色のみ変更（文字色は変更しない）
- ハイライト適用前の背景色を `dataset.origBg`、文字色を `dataset.origColor` に保存し、解除時に復元する
...
- 解除バグ修正 (2026-06-12): 解除時に元のスタイルに戻らなかった問題を修正（`dataset.origBg`/`origColor` 保存・復元方式）
- 文字色: 黒アバター時のみ白文字に確定（それ以外は文字色変更なし）

---

### 10. ログ複数行入力（Shift+Enter）✅ (2026-04-13 実装完了)
...
### 11. 他者からのアバター落書きON/OFF ✅ (2026-04-13 実装完了)
...
### 16. カメラプレビュー（配信中カメラ切替）✅ (2026-05-31 実装完了)
...
### 18. 複数カメラ ✅ (2026-06-12 実装完了、2026-06-13 バグ修正、2026-06-13 追加バグ修正)
...
#### 追加実装済み ✅ (2026-06-10 実装完了)
- ダブルクリック（PC）でも透過/通常モード切替（`pointerup` 300ms二連打で `toggleVideoTransparent()`）
- PC: 動画上でマウスホイールスクロールすると透過度が変わる（`v.addEventListener('wheel', ...)`）
...
### 25. 特定部屋への直接リンク ✅ (2026-05-17 実装完了)
**Direct room link**
...
### 28. 配信画面の足場化（アバター乗り上げ）✅ (2026-06-10 実装完了)
...
### 29. アバター同士乗車の全クライアント同期バグ修正 ✅ (2026-05-31 修正完了)
...
### 30. パフォーマンス改善：ticker管理 ✅ (2026-05-31 修正完了)
...
### 32. アバター乗車物理・ジッターバグ修正 ✅ (2026-06-01 修正完了)
...
### 36. ウィンドウ変更時の画面チャット自動表示バグ修正 ✅ (2026-06-04 実装完了)
...
### 37. スマホ配信時の動画上アバター細問題修正 ✅ (2026-06-10 実装完了)
...
### 38. むげん部屋キーボード移動ループ ✅ (2026-06-11 最終更新)
...
### 40. 編集用UIをF12コンソール開いてる時だけ表示 ✅ (2026-06-11 実装完了)
...
### 42. usersボタン → 部屋人数ボタン改修 ✅ (2026-06-11 実装完了)
...
### 43. 部屋入退室通知に部屋名を表示 ✅ (2026-06-04 実装完了（未確認）)
...
**バグ修正（2026-06-13）**
- `_applyDirectionBg`: `PIXI.Texture.from()` は非同期でテクスチャをロードするため、ロード前に `sp.width/height` を設定するとPIXIがロード後にスケールを再計算して画像が巨大になる問題を修正。`tex.baseTexture.valid` を確認し、未ロードなら `tex.baseTexture.once('loaded', ...)` でロード完了後にサイズを設定するよう変更。

**BGプリロード方式改修（2026-06-13）**
- 問題: 方角部屋入室時に「東/西/南/北」の漢字が一瞬見えてから背景画像が貼られるフラッシュが発生
- 旧実装: ページロード時（ログイン前）にBG URL + 画像をフェッチ → セキュリティ上ログイン前に画像URLが見える問題
- 新実装: **ログイン後にバックグラウンドでプリロード開始** → 両問題を同時解決
  - `_startDirBgPreloads()`: ログイン時に全4部屋のBGをfetch + canvas経由でPIXIスプライト生成（`_dirBgPreloadPromises` に格納）
  - 通常移動（エントランス→ワープ→方角部屋）: エントランスにいる間にロード完了するため `await` はゼロ待ち → フラッシュなし
  - 直リンク: ログイン後すぐに対象部屋のBGを `await` してから `displayRoom()` → フラッシュなし（初回のみ画像DL分のラグあり）
  - `goSelfToRoomSpot` を `async` 化: await後に旧部屋削除→新部屋表示の順にすることでブランクフレームも防止
  - BG更新時は `delete _dirBgPreloadPromises[roomName]` + `_startDirBgPreloads()` で即再プリロード
...
### 48. 更新履歴改修 ✅ (2026-06-20 実装完了)
**Changelog update**
- 更新履歴ページに spec.md を見れる URL を掲載（`/spec` で raw テキスト配信）

---

### 49. カンパ用URLページ ✅ (2026-06-20 実装完了)
**Donation page**
- https://ko-fi.com/nmachi と Amazon ギフト券ページへのリンクを掲載したページを新規作成

#### 現状
- フッターの「リンク集☆」横に「カンパ」リンクを追加（`views/index.ejs`）
- リンク先: https://ko-fi.com/nmachi
- Q&A・リンク集☆の下端にフォントサイズそのままで下詰め揃い

---

### 50. キーボード移動で画面外に行けるバグ修正 ✅ (2026-06-04 実装完了)
...
**現状（2026-06-13 時点で実施済み）**
- 方角部屋（東西南北）入室時に無駄なAPIコール4本（warp/images/scale-zones/code）をスキップ（`_SYSTEM_ROOM_NAMES` でシステム部屋を除外）
- 方角部屋BGのURLキャッシュ（`_dirBgUrlCache`）: 2回目以降の入室でAPIコールなし
- 方角部屋BGをログイン後すぐにバックグラウンドプリロード（`_startDirBgPreloads()`）: 通常移動ではエントランス滞在中にロード完了 → 入室時フラッシュなし。直リンク時のみ画像DL完了を待ってから表示。セキュリティ上ログイン前にはフェッチしない（旧実装はログイン前にフェッチしていた）
- 部屋リンク直リンク時にエントランス経由を廃止（1往復削減）
- 直リンク先がユーザー部屋の場合、ページ読み込み直後にHTTPキャッシュを先読み
- 方角部屋BG画像をsharp WebP quality 92・最大1320×960でアップロード時圧縮（`routes/direction.js`）
...
**実施済み（2026-06-13）**
- 方角部屋BG画像のURLがログイン前にDevToolsのネットワークタブで見えていた問題を解消（#51と同時対応）: プリロードをログイン後に移行

---

### 53. 動画数変更時に位置をリセット（videoAutoReset）
- 動画の追加・削除時に全動画の `freeFloat` を false にして位置をリセットする
- 現在はコメントアウト中
- 有効化時: `views/index.ejs` のチェックボックスのコメントアウトを外す。CLAUDE.md の localStorage キー一覧に `videoAutoReset` を追記すること
