...
### 23.5. 配信画面の足場化（アバター乗り上げ）✅ (2026-05-19 実装完了)
...
**乗り上げ**
- アバターが動画画面の上端に重なったとき、動画の上面をポリゴン床として重力着地する
- 通常の床・足場と同じ移動・ジャンプが可能

**動画移動・リサイズへの追従**
- 動画がドラッグ移動・ピンチリサイズされた場合、乗っているアバターの座標をオフセット差分だけずらして追従させる
- 追従は自分のアバターのみ計算し `sendTransformData` で他者に同期する
- ピクセル座標は `#mediaContainer` の getBoundingClientRect を PIXI ステージ座標に変換して管理

**配信画面消滅時の追い出し**
- `video` が消えた（配信終了・ページ切替等）タイミングで、乗っていたアバターを部屋の右端（x=660）または左端（x=0）の床上に強制移動する
- 移動先は現在位置に近い方の壁寄り床座標とする
- チャットログに「配信画面が消えました」等のメッセージは表示しない（静かに追い出す）

**配信者の許可/拒否設定**
- 設定パネルに「配信画面に乗ることを許可」チェックボックスを追加（デフォルト ON）
- OFF にすると `mediaContainer` の足場判定を無効化し、他者アバターも乗れなくなる
- 設定変更は Socket.io で部屋内全員に即時反映（`streamSurfaceAllowed` イベント）
- localStorage キー: `streamSurfaceAllowed`（bool）

#### 技術メモ（実装時参照）
- 動画位置は `_videoTransparentActive`（透過モード）か通常モードかで座標計算が異なる
- PIXI座標への変換: `(domX - canvasRect.left) / canvasRect.width * 660` 等
- 動画足場のポリゴンは毎フレーム動画の `getBoundingClientRect` から再計算（移動追従のため）
- `freeFloat=true` の動画が動いた差分を乗っているアバターに加算する
- 複数配信者がいる場合は各動画が独立した足場になる
- 透過モード中は足場無効（画面全体を覆うため着地座標が不定になる）

#### 未決定事項
- 複数人が同時に乗ったとき横並びを自動調整するか（現状は既存の衝突判定に任せる想定）
