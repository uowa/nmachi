...
| # | タスク | ブロック | 理由 |
|---|---|---|---|
| 39 | 部屋入退室通知に部屋名を表示（ID→部屋名） | F / index.js | 実装済みだが動作未確認 |
...
| 24 | 動画数変更時に位置をリセット（videoAutoReset） | index.ejs / index.js | コメントアウト中。有効化時は index.ejs のチェックボックスのコメントアウトを外す。CLAUDE.md の localStorage キー一覧に `videoAutoReset` を追記すること |
...
### 33. スマホ配信時の動画上アバター細問題修正 ✅ (2026-06-10 実装完了)
