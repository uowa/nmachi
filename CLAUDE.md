...
| タイトルロゴランダム選出・サイト名アンケート | `index.js` `titleFontFamily` 配列（約40フォント、L342）・`title.style.fontFamily` 設定（L1680）・konaTextは配列で fallback "みちます" 指定、`views/index.ejs` 設定パネル内 `#titlePoll`（プレビュー/投票/カスタム追加/円グラフ）、`routes/titleVote.js`（POST `/` POST `/suggest` GET `/options` GET `/counts`）、`db/init.js` `title_votes`/`title_suggestions` テーブル、サンプルプレビュー用 `routes/{nyao,necomachi,nyaomachi,nekomachi,nekomachi-hk,nekomachi-hira,nekomachi-kata}.js`、比較ページ `views/font-test.ejs`、`public/stylesheets/style.css` に各フォントの `@font-face` + `unicode-range`（glyph空フォントのフォールバック制御用）、`public/stylesheets/font-licenses/` にOFL/Apache等のライセンスファイル、配布NGフォントは `.gitignore` で除外し README.md に手順 |
| SE Audio Lazy 生成 | `index.js` `_lazyAudio(src, initialVolume)` (L229) → 初回 `.play()` 時に `new Audio()` 生成。`.volume` setter は pendingVolume を蓄え play 時に反映。`msgSE` 全 27 エントリで利用 |
| 直リンク・部屋名キャッシュ | `index.js` `_UUID_RE` (L131) / `_directLinkRoomInfo` (L129) / `_userRoomNameCache` (L130) / `_directLinkResolvePromise` (L128)、`routes/rooms.js` `/api/rooms/resolve` は name/avatar_scale/has_password も返す、login() の直リンクユーザー部屋分岐で `_userRoomDisplayName` を同期セット (L5694) |
| 部屋リンク表示・コピーフィードバック | `index.js` `updateRoomLinkDisplay()` / `copyRoomLink()`、`views/index.ejs` `#roomLinkInput` (display 用未エンコード) / `#roomLinkCopyBtn` / `#roomLinkCopyToast` (動的生成、1.5s で fade)、`input.dataset.copyUrl` に URL エンコード版を保持 |
| ユーザー部屋画像の事前 fetch | `index.js` `_prewarmWarpTargets(zones)` (L666) / `_prewarmedRoomIds` Set。`loadDbWarpZones` 末尾で起動。target が UUID 形式なら `/api/rooms/:id` (名前キャッシュ) + `/api/rooms/:id/images` (画像URL を `new Image().src` でプリロード) |
| 無音オーディオ unlock | `views/index.ejs` `<audio id="muonAudio">` + `sound/etc/muon.wav` (444B silent WAV)、`index.js` L1714 の `pointerdown once: true` で play()。旧 muon.mp3 (213KB) は 2026-06-26 削除済み |
...
- **git オブジェクト大規模破損（2026-05-06・2026-06-20 二度修復、2026-06-23 reflog残骸も掃除済み）**: Google Drive 同期が `.git/objects/pack/` を破損。2026-06-20: `git filter-branch` + `git replace` で破損コミットを切り捨て・書き換えし、GitHub に force push で上書き。2026-06-23: `git reflog expire --expire=now --all` + `git gc --prune=now` で reflog 残骸と到達不能オブジェクトを掃除（コミット時の `geometric-repack` 失敗エラーは解消、`.git` は 48MB）。現在 97コミット（`0e9ef8d 404ページにやせアボン画像追加` 以降）が有効履歴。それ以前の履歴は破損のため消失済み。
...
- localStorage のキー一覧: `userName`, `avatarAspect`, `avatarUserName`, `colorCode`, `myOekaki`, `sit`, `fontSize`, `mainLogHeight`, `volume`, `PMsize`, `useTTS`, `ttsMode`, `ttsVolume`, `showJoinLeaveMsg`, `oekakiPerStateMode`, `useLogHighlight`, `useAvatarHighlight`, `useLogItemHighlight`, `videoSize`, `videoWidth`, `videoHeight`, `videoReverse`, `videoInverse`, `videoInverseAndReverse`, `videoReverseOther`, `videoInverseOther`, `videoInverseAndReverseOther`, `bubbleOffsetX`, `bubbleOffsetY`, `streamQualityLevel`, `receiverQuality`, `contextMenuPos`, `videoTransparentDefault`, `videoTransparentOpacity`, `cameraSelectMode`, `cameraDeviceId`, `cameraDeviceLabel`, `micSelectMode`, `micDeviceId`, `micDeviceLabel`, `showCoord`, `useRNNoise`, `micBoost`, `lowCut`, `cameraDeviceId_2`〜`cameraDeviceId_10`（cam2+のデバイスID、連番なので途中が空なら以降も空）、`showMainLog`（下ログ表示状態 "1"/"0"）、`showOverlayChat`（画面チャット表示状態 "1"/"0"、未設定時はスマホ初回起動で自動ON）、`titleVoteToken`（サイト名アンケート用の匿名UUID、初回投票時に自動生成）
